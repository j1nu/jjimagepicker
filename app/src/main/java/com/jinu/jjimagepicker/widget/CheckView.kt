...
open class CheckView : View {
    ...
    protected open fun init(context: Context) {
        ...
        mCheckDrawable = ResourcesCompat.getDrawable(context.resources,
                R.drawable.ic_check, context.theme)
    }

    fun setChecked(checked: Boolean) {
        if (mCountable) {
            throw IllegalStateException("com.jinu.jjimagepicker.view.CheckView is countable, call setCheckedNum() instead.")
        }
        mChecked = checked
        invalidate()
    }
    ...
    fun setCheckedNum(checkedNum: Int) {
        if (!mCountable) {
            throw IllegalStateException("com.jinu.jjimagepicker.view.CheckView is not countable, call setChecked() instead.")
        }
